...
/**
 * Dothat problem that appeared : 
 * 	- 
 */

Babelute.dothatInitializer = function(lexicName, actionsName) {
	var lexic = Babelute.getLexic(lexicName),
		actions = Babelute.getActions(actionsName),
		Dothat = function(subject, env) {
			lexic.Cl.call(this);
			this._subject = subject;
			this._env = env;
		};

	Dothat.prototype = new lexic.Cl();
	Dothat.prototype._babelute = null;
	Dothat.prototype._append = function(lexic, name, args) {
	...
	return function(subject, env) {
		return new Dothat(subject, env || new Environment(actionsName));
	};
};
